        ...
        if isinstance(event.message, sirius_sdk.aries_rfc.OfferCredentialMessage):
            ...
            if success:
                ...
                if feature_0037.problem_report:
                    print('Prover: problem report:')
                    print(json.dumps(feature_0037.problem_report, indent=2, sort_keys=True))
...
async def run():
    ...
    while True:
        ...
        if case == '1':
            ...
            str_data = base64.b64decode(str_data.encode('utf8') + b'=' * (-len(str_data) % 4)).decode()
            if str_data[-1] != '}':
                str_data = str_data + '}'
            inv_json = json.loads(str_data)
            invitation = Invitation(**inv_json)
            await connection_routine(invitation)
            break
        elif case=='2':
            creds = await sirius_sdk.AnonCreds.prover_get_credentials({})
            print(creds)
        elif case == 'exit':
            break
        else:
